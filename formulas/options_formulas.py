...
def binomial_dividend_pricing(
    ...
    try:
        ...
        logger.info(
            f"Underlying Price: {underlying_price}, Strike Price: {strike_price}, Time to Maturity: {time_to_maturity}"
        )
        logger.info(
            f"Risk-Free Rate: {risk_free_rate}, Volatility: {volatility}, Dividend Yield: {dividend_yield}, Steps: {steps}"
        )
        ...
        logger.info(
            f" Binomial pricing with dividends completed. Result = {option_price}"
        )

        return {
            "option_price_with_dividend": (
...
def black_scholes_pricing(
    ...
    try:
        ...
        dividend_yield = float(
            dividend_yield if dividend_yield not in [None, ""] else 0.0
        )

        # Ajustement du prix sous-jacent pour le dividende
        adjusted_underlying_price = underlying_price * exp(
            -dividend_yield * time_to_maturity
        )

        # Calcul des termes d1 et d2
        d1 = (
            log(adjusted_underlying_price / strike_price)
            + (risk_free_rate + (volatility**2) / 2) * time_to_maturity
        ) / (volatility * sqrt(time_to_maturity))
        d2 = d1 - volatility * sqrt(time_to_maturity)

        if option_type == "CALL":
            price = adjusted_underlying_price * norm.cdf(d1) - strike_price * exp(
                -risk_free_rate * time_to_maturity
            ) * norm.cdf(d2)
        elif option_type == "PUT":
            # Utiliser la parité put-call : Put = Call - S * exp(-qT) + K * exp(-rT)
            call_price = adjusted_underlying_price * norm.cdf(d1) - strike_price * exp(
                -risk_free_rate * time_to_maturity
            ) * norm.cdf(d2)
            price = (
                call_price
                - adjusted_underlying_price
                + strike_price * exp(-risk_free_rate * time_to_maturity)
            )
        else:
    ...
    try:
        ...
        def bs_price(volatility):
            ...
            d1 = (
                ...
                + (risk_free_rate - dividend_yield + (volatility**2) / 2)
                * time_to_maturity
            ) / (volatility * sqrt(time_to_maturity))
            d2 = d1 - volatility * sqrt(time_to_maturity)
        ...
        for i in range(num_simulations):
            ...
            if option_style == "European":
                ...
                if option_type == "CALL":
                    ...
                    antithetic_payoffs[i] = max(
                        0, antithetic_average_price - strike_price
                    )
                elif option_type == "PUT":
                    payoffs[i] = max(0, strike_price - average_price)
                    antithetic_payoffs[i] = max(
                        0, strike_price - antithetic_average_price
                    )
                else:
                    raise ValueError("Invalid option type. Must be 'CALL' or 'PUT'.")
...
def compare_american_vs_european(
    ...
    try:
        american_price = binomial_dividend_pricing(
            option_type,
            "AMERICAN",
            underlying_price,
            strike_price,
        ...
        european_price = binomial_dividend_pricing(
            option_type,
            "EUROPEAN",
            underlying_price,
            strike_price,
...
def plot_greeks_sensitivity_formula(
    option_type,
    underlying_price,
    strike_price,
    time_to_maturity,
    ...
    for S in price_range:
        ...
        if option_type == "CALL":
            ...
            call_price = exp(-dividend_yield * time_to_maturity) * S * norm.cdf(
                d1
            ) - exp(-risk_free_rate * time_to_maturity) * strike_price * norm.cdf(d2)
        else:
            delta.append(-exp(-dividend_yield * time_to_maturity) * norm.cdf(-d1))
            call_price = exp(
                -risk_free_rate * time_to_maturity
            ) * strike_price * norm.cdf(-d2) - exp(
                -dividend_yield * time_to_maturity
            ) * S * norm.cdf(
                -d1
            )
    ...
    return {
        ...
        "secondary_y_axis": {
            "label": "Option Value",
            "value": list(
                map(lambda x: round(x, 2), call_value)
            ),  #  Ajout d'une échelle secondaire
        },
        "delta": {
            "label": "Delta",
