document.addEventListener("DOMContentLoaded", () => {
  ...
  const fetchActions = async (searchTerm = "", limit = 4) => {
    try {
      const response = await fetch(
        `/api/stocks?search=${searchTerm}&limit=${limit}`,
      );
  ...
  const selectStock = async (stock) => {
    ...
    const existingStock = Array.from(actionsList.children).some(
      (child) =>
        JSON.parse(child.dataset.action || "{}").ticker === stock.ticker,
    );
    ...
    const newAction = {
      ...
      price:
        stockDetails.price !== null && !isNaN(stockDetails.price)
          ? stockDetails.price
          : "N/A",
      change:
        stockDetails.change !== null && !isNaN(stockDetails.change)
          ? stockDetails.change
          : "N/A",
      change_monthly:
        stockDetails.change_monthly !== null &&
        !isNaN(stockDetails.change_monthly)
          ? stockDetails.change_monthly
          : "N/A",
    };
  ...
  window.openSeeMore = async (ticker) => {
    ...
    if (stockDetails) {
      ...
      document.getElementById("stock-industry").textContent =
        `Industry: ${stockDetails.industry || "N/A"}`;
      document.getElementById("stock-market-cap").textContent =
        `Market Cap: ${formatNumber(stockDetails.market_cap)}`;
      document.getElementById("stock-exchange").textContent =
        `Exchange: ${stockDetails.exchange || "N/A"}`;
      ...
      document.getElementById("stock-52-week-high").textContent =
        `52-Week High: $${stockDetails["52_week_high"] || "N/A"}`;
      document.getElementById("stock-52-week-low").textContent =
        `52-Week Low: $${stockDetails["52_week_low"] || "N/A"}`;
      document.getElementById("stock-day-high").textContent =
        `Day High: $${stockDetails.day_high || "N/A"}`;
      document.getElementById("stock-day-low").textContent =
        `Day Low: $${stockDetails.day_low || "N/A"}`;
      ...
      document.getElementById("stock-forward-pe").textContent =
        `Forward P/E: ${stockDetails.forward_pe || "N/A"}`;
      document.getElementById("stock-peg-ratio").textContent =
        `PEG Ratio: ${stockDetails.peg_ratio || "N/A"}`;
      document.getElementById("stock-price-to-book").textContent =
        `P/B Ratio: ${stockDetails.price_to_book || "N/A"}`;
      ...
      document.getElementById("stock-roe").textContent =
        `Return on Equity (ROE): ${formatPercentage(stockDetails.return_on_equity)}`;
      document.getElementById("stock-profit-margins").textContent =
        `Profit Margins: ${formatPercentage(stockDetails.profit_margins)}`;
      document.getElementById("stock-operating-margins").textContent =
        `Operating Margins: ${formatPercentage(stockDetails.operating_margins)}`;
      ...
      document.getElementById("stock-dividend-rate").textContent =
        `Dividend Rate: $${stockDetails.dividend_rate || "N/A"}`;
      document.getElementById("stock-payout-ratio").textContent =
        `Payout Ratio: ${formatPercentage(stockDetails.payout_ratio)}`;
      ...
      document.getElementById("stock-target-price").textContent =
        `Target Price: $${stockDetails.target_mean_price || "N/A"}`;
      document.getElementById("stock-target-high").textContent =
        `Target High: $${stockDetails.target_high_price || "N/A"}`;
      ...
      ensureChartRender(
        ctx,
        stockDetails.chart_data.dates,
        stockDetails.chart_data.prices,
  ...
  const renderChart = (ctx, dates, prices, volumes) => {
    ...
    if (
      ...
      console.error(
        "Chart data is missing or empty. Skipping chart rendering.",
      );
      return;
    }
  ...
  const ensureChartRender = (ctx, dates, prices, volumes) => {
    if (
      ...
      console.error(
        `Skipping chart rendering: Missing data for ${ctx.canvas.id}`,
      );
      ctx.canvas.parentNode.innerHTML =
        "<p style='color:red;'>Chart unavailable</p>";
      return;
    }
...
`;
  ...
  window.openSeeNews = async (ticker) => {
    ...
    if (newsArticles.length === 0) {
      ...
      document.getElementById("modal-news-title").textContent =
        `Latest News for ${ticker}`;
      document.getElementById("news-container").innerHTML = newsArticles
        .map(
          (article) => `
                <div class="news-item">
