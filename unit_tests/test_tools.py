...
def generate_input_payloads(tool_config):
    ...
    for inp in input_defs:
        ...
        if inp.get("data_target"):
            logger.info(
                f"Skipping input '{inp['id']}' because it's a file upload input"
            )
            continue
        ...
        if inp["type"] == "select":
            ...
            if placeholder is not None:
                try:
                    ...
                    if step == "any":
                        placeholder = float(
                            placeholder.replace("(", "").replace(")", "").split()[0]
                        )
                except:
                    pass
...
def write_csv_report(results, filename):
    with open(filename, mode="w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(
            ["Tool Name", "Input Payload", "Status Code", "Passed", "Error"]
        )
        ...
        for result in results:
            ...
            writer.writerow([tool_name, input_payload, status_code, passed, error])
